...
#### data
data <- Boston
#write.csv(data, '/Users/apple/Documents/UCDSmurfit/3_Statistical_learning/assignment2/Boston.csv')
dim(Boston)
...
# define data
x.train <- model.matrix(crim~., data.train)[, -1]
y.train <- data.train$crim
x.test <- model.matrix(crim~., data.test)[, -1]
y.test <- data.test$crim
...
summary(lm.fit)
# for 5% of the confidence level, the null hypothesis can be rejected for the predictors:
# 'rad', 'dis', 'medv', 'zn', 'nox'.
prediction.lm <- predict(lm.fit, data.test)
# plot comparison between prediction and ture value
plot(data.test$crim, ylab='Crime Rate', type='l', main='Comparison of Prediction & True Value with MLR', col='black')
lines(prediction.lm, col = 'red')
legend('topright', legend=c('True Value', 'Prediction'), fill =c('black', 'red'))
# calculate and plot residuals
residuals.lm <- prediction.lm - data.test$crim
plot(residuals.lm, ylab='Residual', main = "Residuals with MLR")
# calculate mse
mse.lm <- mean((prediction.lm - data.test$crim)^2)
mse.lm
...
# plot comparison between prediction and ture value
plot(data.test$crim, type='l', main='Comparison of Prediction & True Value with Ridge', col='black')
...
# plot comparison between prediction and ture value
plot(data.test$crim, type='l', main='Comparison of Prediction & True Value with Ridge.CV', col='black')
...
ridge.coef <- predict(cv.ridge.mod, type = 'coefficients', s = bestlam.ridge)[1:14,]
ridge.coef
...
# plot comparison between prediction and ture value
plot(data.test$crim, type='l', ylab='Crime Rate', main='Comparison of Prediction & True Value with LASSO', col='black')
...
# plot comparison between prediction and ture value
plot(data.test$crim, type='l', ylab='Crime Rate', main='Comparison of Prediction & True Value with LASSO.CV', col='black')
...
# use the best model (with cross validataion) to get coefficients
lasso.coef <- predict(cv.lasso.mod, type = 'coefficients', s = bestlam.lasso)[1:14,]
lasso.coef
...
# plot comparison between prediction and ture value
plot(data.test$crim, type='l', ylab='Crime Rate', main='Comparison of Prediction & True Value with ElasticNet', col='black')
...
# plot comparison between prediction and ture value
plot(data.test$crim, type='l', ylab='Crime Rate', main='Comparison of Prediction & True Value with ElasticNet.CV', col='black')
...
# use the best model (with cross validataion) to get coefficients
elastic.coef <- predict(cv.elastic.mod, type = 'coefficients', s = bestlam.cv.elastic)[1:14,]
elastic.coef
